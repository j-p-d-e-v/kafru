...
impl<'a> Queue<'a>{
    ...
    pub async fn list(&self,status:Vec<String>, limit: Option<u64>) -> Result<Vec<TaskData>,String> {
    ...
    pub async fn push(&self, mut data: TaskData ) -> Result<TaskData,String> {
        let id: String = uuid::Uuid::new_v4().to_string();
        data.date_created = Some(Utc::now());
        data.status = Some(TaskStatus::Waiting);
        match self.db.client.create::<Option<TaskData>>((self.table,id)).content(data).await {
            Ok(result) => {
    ...
    pub async fn remove(&self, id: Thing ) -> Result<TaskData,String> {
        match self.db.client.delete::<Option<TaskData>>((self.table,id)).await {
            Ok(result) => {
                if let Some(record) = result {
                    return Ok(record);
                }
    ...
    pub async fn get(&self, id: Thing) -> Result<TaskData,String> {
        match self.db.client.select::<Option<TaskData>>((self.table,id)).await {
            Ok(data) => {
    ...
    pub async fn update(&self, id: Thing, data: TaskData ) -> Result<TaskData,String> {
        match self.get(id.clone()).await  {
            Ok(record)=> {
                ...
                match self.db.client.update::<Option<TaskData>>((self.table,record.id.unwrap())).content(data).await {
                    Ok(result) => {
                        if let Some(record) = result {
                            return Ok(record);
                        }
